...
const mangaRecomendacion = document.querySelector('#mangaRecomendacion')

    const random = Math.floor(Math.random() * 5000);
    fetch('https://kitsu.io/api/edge/manga/'+random)
    .then((response) => response.json())
    .then((data) => {
        const li = document.createElement('li')
        li.innerHTML = `<h4>${data.data.attributes.canonicalTitle}</h4>
        <img src="https://media.kitsu.io/manga/poster_images/${random}/small.jpg?1491099787">
        <label><b>${data.data.attributes.synopsis}</b></label>
        `
        mangaRecomendacion.append(li)
    });
